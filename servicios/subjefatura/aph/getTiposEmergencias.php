...
try {
    ...
    if (!isset($_GET['id'])) {
        ...
        $database->query("SELECT * FROM subjefatura.tb_aph_hojaruta_tipoemergencia");
        ...
        $database = new Database();
        $database->query("SELECT * FROM subjefatura.tb_aph_hojaruta_tipoemergencia WHERE aph_hojaruta_tipoemergencia_id = :id");
        $database->bind('id', $id);
