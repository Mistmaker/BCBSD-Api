...
try {
    ...
    if (!isset($_GET['id'])) {
        ...
        $database->query("SELECT * FROM subjefatura.tb_voluntarios order by voluntario_id LIMIT 100");
        ...
        $database = new Database();
        $database->query("SELECT * FROM subjefatura.tb_voluntarios WHERE voluntario_id = :id");
        $database->bind('id', $id);
