...
try {
    ...
    if (!isset($request->evento_id)) {
        ...
        $database->query('INSERT INTO subjefatura.tb_voluntarios_eventos (
            evento_estado,
            evento_fecha_inicio,
            evento_hora_inicio,
        ...
        $database->bind(':evento_estado', prop($request, 'evento_estado'));
        $database->bind(':evento_fecha_inicio', prop($request, 'evento_fecha_inicio'));
        $database->bind(':evento_hora_inicio', prop($request, 'evento_hora_inicio'));
        ...
        if (!$inserted) {
            jsonResponse(['error' => 'Error al insertar evento', 'msg' => $database->getErrors()[2]], 500);
        }
        $database->endTransaction();
        $database->closeConnection();
        jsonResponse(['msg' => 'Evento registrado con éxito', 'id' => $inserted['evento_id']], 201);
    } else {
        // Actualizar un voluntario existente
        $database->query('UPDATE subjefatura.tb_voluntarios_eventos SET
        ...
        $database->bind(':evento_id', prop($request, 'evento_id'));
        $database->bind(':evento_estado', prop($request, 'evento_estado'));
        $database->bind(':evento_fecha_inicio', prop($request, 'evento_fecha_inicio'));
        $database->bind(':evento_hora_inicio', prop($request, 'evento_hora_inicio'));
        $database->bind(':evento_fecha_finalizacion', prop($request, 'evento_fecha_finalizacion'));
        ...
        if (!$success) {
            jsonResponse(['error' => 'Error al actualizar evento', 'msg' => $database->getErrors()[2]], 500);
        }
        $database->endTransaction();
        $database->closeConnection();
        jsonResponse(['msg' => 'Evento actualizada con éxito', 'id' => $request->evento_id], 200);
    }
} catch (Throwable $th) {
    // throw $th;
