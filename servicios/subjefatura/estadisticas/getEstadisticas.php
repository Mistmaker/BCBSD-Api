...
try {
    ...
    $sqlTotalIntervenciones = "SELECT COUNT(intervencion_id) as total FROM subjefatura.tb_intervenciones";
    $sqlTotalBeneficiarios = "SELECT SUM(incidencia_beneficiarios) as total_b FROM subjefatura.tb_intervenciones";
    $sqltotalFallecidos = "SELECT SUM(incidencia_fallecidos) as total_f FROM subjefatura.tb_intervenciones";
    ...
    $sqlDiaCritico = "SELECT 
            ...
            COUNT(*) AS total
        FROM subjefatura.tb_intervenciones
        GROUP BY dia_semana
        ORDER BY total DESC
        LIMIT 1;";
    ...
    $chartCausas = [
        ...
        "options" => $chartOptions,
        "data" => [
            "labels" => array_column($rows_causas, "causa"),
            "datasets" => [
                [
                    "label" => "Causas",
                    "data" => array_column($rows_causas, "cant"),
                    "backgroundColor" => array_slice($colors, 0, count($rows_causas))
                ]
            ]
        ]
    ];
    ...
    // $labels = array_column($rows_tipo_incidencias, 'tipo');
    // $dataPoints = array_map(fn($row) => (int)$row['cant'], $rows_tipo_incidencias);
    // // Colores por punto (uno por incidencia)
    // $pointColors = array_map(function ($i) use ($colors) {
    //     return $colors[$i % count($colors)];
    // }, array_keys($rows_tipo_incidencias));
