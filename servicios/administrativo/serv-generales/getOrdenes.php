...
try {
    ...
    , case when orden_hora_entrada is not null then FLOOR(EXTRACT(EPOCH FROM (orden_hora_entrada - orden_hora_salida)) / 3600)::text || ' H ' || LPAD(FLOOR(MOD(EXTRACT(EPOCH FROM (orden_hora_entrada - orden_hora_salida))::decimal, 3600) / 60)::text , 2, '0') else 0::text end as horas_orden
    ...
    if (!isset($_GET['id'])) {
        ...
        $database = new Database();
        $database->query("SELECT * FROM logistica.tb_ordenesmovilizacion WHERE orden_id = :id");
        $database->bind('id', $id);
